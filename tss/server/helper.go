...
func (s *MPCServer) publishToRedis(sessionID string, data map[string]interface{}, channel string) error {
	...
	s.logger.Debug("published result to Redis", zap.String("channel", channel), zap.String("session_id", sessionID))
	return nil
}
...
func (s *MPCServer) getOrCreateParty(sessionID string) *party.Party {
	...
	s.logger.Debug("created new party", zap.String("session_id", sessionID))
	return p
}
